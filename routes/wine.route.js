...
router.get("/bottles", async (req, res) => {
  try {
    ...
    const database = client.db("bottles");
    const collection = database.collection("bottles");
    
    // Find documents
    const query = { saleStatus: false };
    const bottles = await collection.find(query).toArray();
...
router.get("/userBottles", (req, res) => {
  ...
  WineModel.find({
    $and: [{ saleStatus: false }, { userSeller: req.session.loggedInUser._id }],
  })
...
router.get("/userBottlesSold", (req, res) => {
  WineModel.find({
    $and: [{ saleStatus: true }, { userSeller: req.session.loggedInUser._id }],
  })
    .then((sales) => {
      console.log(req.session.loggedInUser._id);
      sales.forEach((eachsales) => {
        console.log(eachsales.saleStatus, eachsales.userSeller);
      });
...
router.get("/userBottles/:userId", (req, res) => {
  ...
  WineModel.find({
    $and: [{ saleStatus: false }, { userSeller: req.params.userId }],
  })
